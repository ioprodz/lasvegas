...
struct HarmonicState {
    ...
    active_count: usize,
    block_types: [usize; 6],
    /// Per-block direction: false = forward, true = reversed
...
impl HarmonicState {
    ...
    fn apply_fingerprint(&mut self) {
        ...
        self.active_count = 2 + ((fp & 0xFF) as usize % 5);
        ...
        // Per-block direction from bits 48-53 (one bit per block)
        for i in 0..6 {
...
/// Harmonic Memory V3 — like V1 but with per-block direction control.
/// Each block's animation direction (forward/reversed) is deterministically
/// derived from the chord progression fingerprint. Adjacent blocks often
/// run opposite directions, creating visual counterpoint.
pub fn audio_harmonic3(strip: &mut LedStrip, _frame: usize, a: &AudioAnalysis) {
    STATE.with(|state| {
        ...
        for t in s.travelers.iter() {
            ...
            for i in 0..((tail_len + 1.0) as usize) {
                ...
                for dr in -1i32..=1 {
                    ...
                    add_color(strip, idx, hsv_to_rgb(t.hue, 0.85, b));
                }
            }
        }
        ...
        for slot in 0..active_count {
            ...
            match block_type {
                0 => {
                    // ── PULSE ──
                    // Reversed: phase runs backward for counter-breathing
                    let phase = PULSE_PHASE[slot % 6];
                    let pulse_mod =
                        (frame as f32 * 0.08 * s.speed_mult * dir + phase).sin() * 0.5 + 0.5;
                ...
                2 => {
                    // ── SPARKLE ── (sparkle is non-directional, no change needed)
                ...
                4 => {
                    // ── ENERGY BAR ──
                    // Reversed: bar grows from opposite side
                    ...
                    for row in 0..ROWS {
                        for col_off in 0..block_width {
                            ...
                            // If reversed, shift the center point to the opposite side
                            let center = if reversed {
                                block_width - 1 - half
                            } else {
                                half
                            };
                            let dist = if col_off >= center {
                                col_off - center
                            } else {
                                center - col_off
                            };
                ...
                5 => {
                    // ── WAVE ──
                    // Reversed: wave travels in opposite direction
                    ...
                    let speed = 0.06 * s.speed_mult * dir;
...
/// Chase with optional reversal. When reversed, the light travels in the
/// opposite direction within the block.
fn render_chase(
    strip: &mut LedStrip,
    frame: usize,
    col_start: usize,
    ...
    let raw_pos = ((frame + phase) * chase_speed) % block_leds;
    let pos = if reversed {
        block_leds - 1 - raw_pos
    } else {
        raw_pos
    };
